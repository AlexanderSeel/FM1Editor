...
export function validatePhysicalEvidenceConsistency(
  ...
  const jsonArtifacts: NamedJsonArtifact[] = inputs
    ...
    .map((input) => ({ name: input.name, value: input.jsonValue }))
  ...
  if ((packageTarget === 'fm1' || packageTarget === 'mixed') && fm1ManifestCount === 0) {
    addIssue(issues, 'error', 'fm1-manifest-missing', [], 'The selected package target requires an FM-1 hardware evidence manifest.')
  }
  if ((packageTarget === 'dx7' || packageTarget === 'mixed') && dx7ManifestCount === 0) {
    addIssue(issues, 'error', 'dx7-manifest-missing', [], 'The selected package target requires a stock-DX7 hardware evidence manifest.')
  }
  ...
  for (const item of manifests) {
    ...
    const comparisons = monitors.map((monitor) => ({
      monitor,
      mismatchFields: hardwareMidiSummaryMismatchFields(item.manifest.midiCapture, monitor.summary),
    }))
    ...
    if (exact.length === 1) {
      ...
      links.push({ manifestName: item.name, target: item.target, matchedMidiMonitorName: matched.name, summaryMismatchFields: [] })
      ...
      links.push({ manifestName: item.name, target: item.target, matchedMidiMonitorName: null, summaryMismatchFields: [] })
    } else {
      ...
      links.push({ manifestName: item.name, target: item.target, matchedMidiMonitorName: null, summaryMismatchFields: mismatchFields })
    }
  }

  for (const monitor of monitors) {
    if (!usedMonitorNames.has(monitor.name)) addIssue(issues, 'warning', 'unlinked-midi-monitor', [monitor.name], `Raw MIDI export ${monitor.name} is not uniquely linked to a hardware evidence manifest in this package.`)
  }
