...
export function BankBrowser({ voices, selectedVoice, onSelect, onChange }: BankBrowserProps) {
    ...
    <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Drag slots to reorder. Changes stay local until exported.</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
        {voices.map((voice, index) => (
          <button
            className={`rounded-xl border px-3 py-3 text-left transition ${voice === selectedVoice ? 'border-cyan-300/50 bg-cyan-300/10 text-white' : 'border-white/10 bg-black/15 text-slate-400 hover:border-white/25 hover:text-white'}`}
            draggable
