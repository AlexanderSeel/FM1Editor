...
try {
  ...
  const distancePairs=await evaluate(cdp,`[...${root}.querySelector('[aria-label="CMA-ES refined candidates"]').querySelectorAll('article')].map((article)=>[Number(article.getAttribute('data-initial-distance')),Number(article.getAttribute('data-best-distance'))]).filter((pair)=>pair.every(Number.isFinite))`)
