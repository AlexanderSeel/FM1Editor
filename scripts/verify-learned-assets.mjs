...
const EXPECTED_SCALER_DERIVATIVE_SHA256 = '77494299cad04042c96574676eb9a573f534a0e83e2574683b661678b065f532'
...
if (!existsSync(scalerPath)) fail('pinned SpiegeLib scaler JSON is missing')
...
const scalerBytes = readFileSync(scalerPath)
const scaler = readJson(scalerPath, 'SpiegeLib scaler')
if (sha256(scalerBytes) !== EXPECTED_SCALER_DERIVATIVE_SHA256) fail('converted scaler JSON SHA-256 drifted')
...
if (JSON.stringify(scaler?.fitShape) !== '[10,44,13]' || JSON.stringify(scaler?.fitAxis) !== '[0]' || JSON.stringify(scaler?.featureShape) !== '[44,13]') {
  fail('scaler fit/feature metadata drifted')
}
...
console.log('Learned asset audit: model + scaler hashes, provenance, schemas and notices OK')
