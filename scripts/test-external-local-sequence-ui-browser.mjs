...
const resultPath = resolve(argument('--result', `external-local-sequence-ui-${browserName}.json`))

if (!browserExecutable) {
  console.error('Usage: node scripts/test-external-local-sequence-ui-browser.mjs --browser-executable <path> [--browser-name chrome|edge] [--result path]')
  process.exit(2)
}
...
class CdpConnection {
  ...
  send(method, params = {}) {
    ...
    return new Promise((resolvePromise, reject) => { this.pending.set(id, { method, resolve: resolvePromise, reject }); this.socket.send(JSON.stringify({ id, method, params })) })
  }
...
async function clickButton(cdp, text) {
  const clicked = await evaluate(cdp, `(() => { const button = [...document.querySelectorAll('button')].find((candidate) => candidate.textContent?.trim().includes(${JSON.stringify(text)})); if (!button || button.disabled) return false; button.click(); return true; })()`)
  if (!clicked) throw new Error(`Unable to click enabled button containing ${text}`)
}

async function samplePeak(cdp, durationMs) {
  return evaluate(cdp, `(async () => { const analyser = window.__fm1ExternalAnalyser; if (!analyser) throw new Error('External local analyser was not installed'); const data = new Float32Array(analyser.fftSize); let maximum = 0; const deadline = performance.now() + ${durationMs}; while (performance.now() < deadline) { analyser.getFloatTimeDomainData(data); for (const sample of data) { if (!Number.isFinite(sample)) throw new Error('Non-finite external local sample'); maximum = Math.max(maximum, Math.abs(sample)); } await new Promise((resolve) => setTimeout(resolve, 20)); } return maximum; })()`, true)
}
...
try {
  ...
  const debuggerPort = await reservePort(); profileDir = await mkdtemp(resolve(tmpdir(), 'fm1-external-local-sequence-ui-'))
  browserProcess = spawn(browserExecutable, [`--remote-debugging-port=${debuggerPort}`, `--user-data-dir=${profileDir}`, '--headless=new', '--autoplay-policy=no-user-gesture-required', '--mute-audio', '--no-first-run', '--no-default-browser-check', '--disable-background-networking', '--disable-component-update', 'about:blank'], { stdio: ['ignore', 'pipe', 'pipe'] })
  ...
  cdp = new CdpConnection(target.webSocketDebuggerUrl); await cdp.open(); await cdp.send('Runtime.enable'); await cdp.send('Page.enable')

  await cdp.send('Page.addScriptToEvaluateOnNewDocument', { source: `(() => {
    const listeners = new Map();
    const input = {
      id: 'virtual-clock-input', manufacturer: 'FM1 Editor Test', name: 'Virtual MIDI Clock', type: 'input', version: '1', state: 'connected', connection: 'closed',
      open: async () => { input.connection = 'open'; return input; }, close: async () => { input.connection = 'closed'; return input; },
      addEventListener: (type, listener) => { if (!listeners.has(type)) listeners.set(type, new Set()); listeners.get(type).add(listener); },
      removeEventListener: (type, listener) => listeners.get(type)?.delete(listener),
      dispatchEvent: () => true,
    };
    window.__fm1VirtualMidiInput = input;
    window.__fm1MidiPermissionRequests = 0;
    window.__fm1MidiOutputSendCount = 0;
    window.__fm1EmitMidi = (bytes) => { const event = { data: Uint8Array.from(bytes), timeStamp: performance.now() }; for (const listener of listeners.get('midimessage') ?? []) listener(event); };
    const accessListeners = new Map();
    const access = { inputs: new Map([[input.id, input]]), outputs: new Map(), sysexEnabled: true,
      addEventListener: (type, listener) => { if (!accessListeners.has(type)) accessListeners.set(type, new Set()); accessListeners.get(type).add(listener); },
      removeEventListener: (type, listener) => accessListeners.get(type)?.delete(listener),
    };
    Object.defineProperty(navigator, 'requestMIDIAccess', { configurable: true, value: async () => { window.__fm1MidiPermissionRequests += 1; return access; } });
  })();` })
  ...
  const switched = await evaluate(cdp, `(() => { const select = [...document.querySelectorAll('select')].find((candidate) => [...candidate.options].some((option) => option.value === 'external')); if (!select) return false; select.value = 'external'; select.dispatchEvent(new Event('change', { bubbles: true })); return true; })()`)
  if (!switched) throw new Error('Unable to switch sequence clock mode to external')
  await waitForExpression(cdp, `document.body.textContent.includes('External MIDI input clock')`)
  ...
  const result = { ok: true, browserName, browserProduct: version.Browser ?? null, browserUserAgent: version['User-Agent'] ?? null, workletNodeCount, activePeak, silencePeak, ...diagnostics }
