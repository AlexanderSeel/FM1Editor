...
The virtual engine must be described as DX7-compatible and FM-1-inspired until physical comparison proves stronger equivalence. The implemented dependency-free boundary, source audit, semantic MSFA bridge, blockers and measurable spike gates are recorded in [`docs/research/virtual-dx7-renderer-feasibility-spike.md`](./docs/research/virtual-dx7-renderer-feasibility-spike.md), [`docs/research/msfa-source-audit.json`](./docs/research/msfa-source-audit.json), [`docs/validation/msfa-source-audit.md`](./docs/validation/msfa-source-audit.md), [`docs/validation/msfa-voice-bridge.md`](./docs/validation/msfa-voice-bridge.md) and [`docs/validation/virtual-dx7-renderer-boundary.md`](./docs/validation/virtual-dx7-renderer-boundary.md).

- [ ] In a networked checkout, run `npm run audit:virtual-dx7:upstream -- --write-hashes`, review and commit the exact pinned SHA-256 values, then materialize the candidate source set, retain the original Apache-2.0 headers, add the Apache-2.0 license and required NOTICE/attribution material, and pass `npm run audit:virtual-dx7 -- --source-root <pinned-dexed-checkout> --require-hashes` before vendoring. The tuning, MTS-ESP, JUCE and GPL-wrapper boundaries are already excluded from the feasibility source set.
