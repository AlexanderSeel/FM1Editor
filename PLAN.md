...
- [x] Add a local descriptor cache and standardized render-index workflow around the validated deterministic renderer, keyed by engine/render/descriptor identity so unchanged catalog probes avoid rerendering and matching remains independent of physical FM-1 hardware or real-time playback timing. Evidence: [`docs/validation/preset-descriptor-cache.md`](./docs/validation/preset-descriptor-cache.md).
...
- [x] Evaluate Synth_Dexed and `dx7-synth-js` as implementation references/fallbacks with branch-level license, compatibility, fidelity and maintenance audits; neither is admitted as a runtime dependency. Evidence: [`docs/research/virtual-dx7-alternative-engine-audit.md`](./docs/research/virtual-dx7-alternative-engine-audit.md).
- [x] Implement a separate FM-1-inspired effect graph for documented CC `0–23`: filter, reverb, delay, distortion, chorus and phaser using repository-owned native Web Audio components. Evidence: [`docs/validation/fm1-inspired-fx-graph-software.md`](./docs/validation/fm1-inspired-fx-graph-software.md).
- [x] Reuse `Fm1FxState` and the documented value ranges with explicit dry bypass; the repository documents that software effect algorithms, scaling, routing order, stereo behavior and headroom are not known to match physical FM-1 firmware. Evidence: [`docs/validation/fm1-inspired-fx-graph-software.md`](./docs/validation/fm1-inspired-fx-graph-software.md).
