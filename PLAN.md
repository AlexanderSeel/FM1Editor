...
The current reviewed official FM-1 PC-firmware baseline is **V15 · 2026-07-30**. Physical FM-1 sessions must first re-check the official download center and record the exact firmware under test; see [`docs/research/fm1-firmware-baseline.md`](./docs/research/fm1-firmware-baseline.md). The production delivery gate now enforces the reviewed baseline for current-release READY while retaining other firmware as compatibility evidence; software acceptance is recorded in [`docs/validation/fm1-firmware-baseline-delivery-gate.md`](./docs/validation/fm1-firmware-baseline-delivery-gate.md). A newer firmware publication does not by itself validate any MIDI/audio/protocol behavior.
...
- [ ] Record the tested FM-1 firmware version for the observed silent edit buffer after both rejected single-voice approaches; keep isolated Yamaha voice dumps and byte-index parameter streams disabled. The 2026-08-05 observation has no firmware identity and therefore cannot be attributed to V14/V15 or treated as cross-version evidence.
