...
- [ ] Run the current three-way benchmark (retrieval, seeded constrained CMA-ES and local SpiegeLib learned initialization) on the real 2+2+2 isolated-sound set defined by [`docs/validation/real-reference-benchmark-protocol.md`](./docs/validation/real-reference-benchmark-protocol.md), retain poor/failure cases, import the exported receipts into the evidence-set aggregator, complete listening assessments plus learned perceptual notes, and commit aggregate hash/distance/runtime evidence. Synthetic ground-truth comparison, real-reference receipt export, admitted learned-row execution and aggregate software validation are complete; this item now requires actual real isolated recordings and listening evidence.
