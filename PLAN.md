...
The virtual engine must be described as DX7-compatible and FM-1-inspired until physical comparison proves stronger equivalence. The audited distribution, deterministic seeded offline renderer, stateful one-voice WASM/AudioWorklet and ten-minute branded Chrome/Edge real-time gate are complete; evidence is recorded in [`docs/research/virtual-dx7-renderer-feasibility-spike.md`](./docs/research/virtual-dx7-renderer-feasibility-spike.md), [`docs/validation/msfa-distribution-package.md`](./docs/validation/msfa-distribution-package.md), [`docs/validation/msfa-offline-engine-integration.md`](./docs/validation/msfa-offline-engine-integration.md), [`docs/validation/msfa-audioworklet-software.md`](./docs/validation/msfa-audioworklet-software.md), [`docs/validation/msfa-audioworklet-browser-smoke.md`](./docs/validation/msfa-audioworklet-browser-smoke.md) and [`docs/validation/msfa-audioworklet-ten-minute-soak.md`](./docs/validation/msfa-audioworklet-ten-minute-soak.md).

- [ ] Expose an explicit local DX7-compatible preview in the voice workspace: no `AudioContext` before user enable, load/synchronize only the current semantic voice, keep it isolated from Web MIDI/SysEx and provide safe note-off/all-notes-off cleanup without requiring hardware.
- [ ] Add polyphony with deterministic voice allocation/stealing while preserving the validated one-voice path; then add pitch bend, modulation, sustain and aftertouch to the local renderer with focused regression/browser tests.
- [ ] Route preset audition and the browser sequencer to the local engine as explicit alternatives to hardware MIDI; keep local playback and device transmission separate actions.
...
- [ ] Add deterministic render regression tests, effect-isolation tests, integrated AudioWorklet underrun/error checks and Chrome/Edge performance limits as the renderer expands beyond the accepted one-voice core.
