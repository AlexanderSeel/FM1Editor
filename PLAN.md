...
Known limitations, current evidence and conservative recovery procedures are maintained in [`docs/validation/support-matrix-and-recovery.md`](./docs/validation/support-matrix-and-recovery.md). The in-app **FM-1 delivery evidence gate** deterministically evaluates sanitized Chrome and Edge physical manifests for one HTTPS origin/firmware/editor/Windows tuple; a software-only gate test is not physical evidence. Use the local-only **Physical evidence package** helper documented in [`docs/validation/physical-evidence-package.md`](./docs/validation/physical-evidence-package.md) to SHA-index the exact sanitized artifact set used for a closure proposal; the package index is integrity metadata, not a physical PASS.
...
The staged technical decision and reviewed libraries/research are documented in [`docs/research/audio-to-fm-and-virtual-synth.md`](./docs/research/audio-to-fm-and-virtual-synth.md). Learned-model admission criteria and the 2026-08-08 SpiegeLib/Sound2Synth review are documented in [`docs/research/learned-initializer-admission.md`](./docs/research/learned-initializer-admission.md).

- [ ] Reproduce and pin one MIT-licensed SpiegeLib `vst-fm-sound-match` pre-trained model, record upstream/model hashes, reproduce preprocessing/inference, document the predicted/frozen Dexed parameter subset and map it deterministically into the current semantic `Dx7Voice` model. Produce a standards-valid 155-byte candidate before enabling the benchmark's learned-initialization row; the existing SpiegeLib checkpoint is a research candidate, not yet a product initializer.
- [ ] Run the implemented benchmark on the real 2+2+2 isolated-sound set defined by [`docs/validation/real-reference-benchmark-protocol.md`](./docs/validation/real-reference-benchmark-protocol.md), import the exported receipts into the evidence-set aggregator, complete listening assessments and commit the aggregate hash/metrics/runtime evidence. Synthetic ground-truth comparison, real-reference receipts and aggregate validation are implemented; learned initialization remains explicitly unavailable in the product benchmark until the pinned model admission/mapping task above is complete.
- [ ] Implement and deploy an optional reconstruction accelerator only if committed real-reference benchmark receipts demonstrate a material runtime/quality need or the admitted learned initializer requires server execution. The disabled-by-default client boundary, capability/policy validation, one-shot SHA-bound consent, bounded automatic-deletion receipt enforcement, returned DX7 validation and fully local fallback are implemented; any service must satisfy [`docs/validation/reconstruction-accelerator-contract.md`](./docs/validation/reconstruction-accelerator-contract.md).
