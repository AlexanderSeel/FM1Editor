...
Known limitations, current evidence and conservative recovery procedures are maintained in [`docs/validation/support-matrix-and-recovery.md`](./docs/validation/support-matrix-and-recovery.md). The in-app **FM-1 delivery evidence gate** deterministically evaluates sanitized Chrome and Edge physical manifests for one HTTPS origin/firmware/editor/Windows tuple; a software-only gate test is not physical evidence. Use the local-only **Physical evidence package** helper documented in [`docs/validation/physical-evidence-package.md`](./docs/validation/physical-evidence-package.md) to SHA-index the exact sanitized artifact set and export the hash-bound raw-MIDI correlation receipt for closure proposals. The correlation helper recomputes capture summaries and requires a unique manifest/raw-MIDI match, but neither the package index nor correlation receipt is a physical PASS.
