...
The virtual engine must be described as DX7-compatible and FM-1-inspired until physical comparison proves stronger equivalence. The source/provenance audit and dry offline feasibility gates have passed; evidence is recorded in [`docs/research/virtual-dx7-renderer-feasibility-spike.md`](./docs/research/virtual-dx7-renderer-feasibility-spike.md), [`docs/research/msfa-source-audit.json`](./docs/research/msfa-source-audit.json), [`docs/validation/msfa-source-hash-execution.md`](./docs/validation/msfa-source-hash-execution.md) and [`docs/validation/msfa-deterministic-reset.md`](./docs/validation/msfa-deterministic-reset.md).

- [ ] Create the distributable audited engine package: add Apache-2.0 license and attribution/NOTICE material, mark every modified Apache source prominently, record derived-source hashes, verify no GPL/JUCE/MTS/tuning-library source is included, and reproducibly publish only the accepted MSFA-compatible WASM/glue plus its source/provenance record.
- [ ] Wire the accepted dry WASM renderer behind the semantic `VirtualDx7Engine` interface with the fixed regression fixture, cancellation and local-only execution; add explicit seeded/reset behavior for sample-and-hold LFO before claiming deterministic coverage for every legal voice.
- [ ] Implement the minimum one-voice `AudioWorklet`: local audio enable/resume, semantic voice load, note on/off, all-notes-off, 128-frame callback handling over the 64-frame MSFA core, lifecycle cleanup and dry output only; record ten-minute Chrome and Edge soak evidence with zero uncaught worklet errors before expanding scope.
- [ ] After the basic worklet is stable, add polyphony, velocity variation, pitch bend, modulation, sustain and aftertouch, then route the virtual piano, preset audition and browser sequencer to local audio without requiring Web MIDI hardware.
...
- [ ] Add deterministic render regression tests, effect-isolation tests, AudioWorklet underrun checks and Chrome/Edge performance limits for the integrated renderer.
