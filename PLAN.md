...
The official FM-1 download page currently exposes **Windows/Mac firmware package V14 · 2026-07-06** and a separate **PC Firmware V09 · 2026-06-24** entry. Their relationship to the firmware identity displayed by a physical FM-1 is not documented, so the repository does not infer a device version from either label. Physical FM-1 sessions must re-check the official download center, record the exact firmware shown by the device and enter that exact identity into the production delivery gate; see [`docs/research/fm1-firmware-baseline.md`](./docs/research/fm1-firmware-baseline.md). The corrected fail-closed behavior is software-validated in [`docs/validation/fm1-firmware-source-snapshot-delivery-gate.md`](./docs/validation/fm1-firmware-source-snapshot-delivery-gate.md). A firmware publication does not by itself validate any MIDI/audio/protocol behavior.

## 1. FM-1 protocol verification

Use [`docs/validation/fm1-hardware-test-protocol.md`](./docs/validation/fm1-hardware-test-protocol.md), [`docs/research/fm1-firmware-baseline.md`](./docs/research/fm1-firmware-baseline.md) and the validated in-app MIDI-monitor hardware evidence recorder before proposing closure of any physical FM-1 item.

- [ ] Record the tested FM-1 firmware version for the observed silent edit buffer after both rejected single-voice approaches; keep isolated Yamaha voice dumps and byte-index parameter streams disabled. The 2026-08-05 observation has no firmware identity and therefore cannot be attributed to either currently published V09/V14 download track or treated as cross-version evidence.
...
Known limitations, current evidence and conservative recovery procedures are maintained in [`docs/validation/support-matrix-and-recovery.md`](./docs/validation/support-matrix-and-recovery.md). The in-app **FM-1 delivery evidence gate v3** composes three fail-closed integrity layers: v1 checks Chrome/Edge physical-manifest completeness and, for current-release delivery, requires an explicit expected device firmware entered by the tester; v2 binds each selected manifest SHA-256 to exactly one structurally consistent raw-MIDI capture SHA-256; v3 additionally requires the same selected correlation receipt to byte-bind the captured outgoing Yamaha 4,104-byte bank payload to one exact `.syx` filename/SHA-256 and requires one unique FM-1 physical-evidence package index per browser session containing those exact manifest/raw-MIDI/bank-SysEx identities plus WAV and screenshot-or-notes evidence. Repeated sends of the same bank are acceptable, but multiple distinct bank payloads in one retained delivery capture are blocked as ambiguous. Chrome and Edge must use distinct manifest, raw-MIDI, package-index and WAV hashes; the deliberately controlled merged-bank `.syx` may legitimately be shared. Software acceptance for the integrity layers is recorded in [`docs/validation/fm1-delivery-evidence-integrity.md`](./docs/validation/fm1-delivery-evidence-integrity.md), [`docs/validation/fm1-delivery-package-integrity.md`](./docs/validation/fm1-delivery-package-integrity.md), [`docs/validation/fm1-bank-sysex-evidence-binding.md`](./docs/validation/fm1-bank-sysex-evidence-binding.md) and [`docs/validation/fm1-firmware-source-snapshot-delivery-gate.md`](./docs/validation/fm1-firmware-source-snapshot-delivery-gate.md). The earlier static-V15 receipt in [`docs/validation/fm1-firmware-baseline-delivery-gate.md`](./docs/validation/fm1-firmware-baseline-delivery-gate.md) is retained as superseded CI history only. None of these integrity layers or firmware checks is a physical PASS.

- [ ] Test Chrome and Edge on Windows with a physical FM-1 over USB MIDI and USB audio, including merged bank import, destination selection, preset recall, recording, note-off recovery and channel selection; test BLE MIDI where browser/platform support permits. Re-check the official firmware source at session start, record the exact firmware identity displayed by the FM-1, enter that identity as the expected release-session firmware and use the same exact device firmware in both browser sessions. For each browser session retain the sanitized hardware manifest, matching raw MIDI export, the exact transmitted merged-bank `.syx`, hash-bound correlation receipt and SHA-indexed FM-1 physical-evidence package containing that exact manifest/raw-MIDI/bank-SysEx chain plus WAV and screen/recovery evidence; import both complete session sets into delivery gate v3 and commit the READY receipt when the physical matrix is genuinely complete.
- [ ] Enable GitHub Pages deployment only after a committed **v3 READY** delivery evidence receipt records the intended HTTPS origin/firmware/editor/Windows tuple and binds both browser sessions through manifest → raw MIDI → exact bank `.syx` → package index, including distinct WAV evidence. Repository-subpath build/PWA/static routing for `/FM1Editor/` is implemented and validated in [`docs/validation/pages-subpath-readiness.md`](./docs/validation/pages-subpath-readiness.md); the delivery-gate implementation itself does not satisfy the physical prerequisite or enable deployment automatically.
